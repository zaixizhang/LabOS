"""
LabOS Virtual Screening Tools -- adapted from the STELLA virtual-screening
tool suite for use inside the LabOS agent framework.
"""
...
from labos.tools.llm import json_llm_call

# Load environment variables from .env
load_dotenv()

# ---------------------------------------------------------------------------
# Resource directory -- every helper that reads a local data file resolves
# paths relative to this root.  Set the LABOS_RESOURCE_DIR env-var to
# override; otherwise falls back to ``resource/`` under the repo root.
# ---------------------------------------------------------------------------
RESOURCE_DIR: str = os.environ.get(
    "LABOS_RESOURCE_DIR",
    os.path.join(
...
def validate_genes(genes: List[str], species: str = "human") -> Tuple[List[str], List[str]]:
    ...
    try:
        hgnc_path = os.path.join(RESOURCE_DIR, "hgnc_name.txt")
        ...
        print(f"HGNC gene list file not found at {hgnc_path}")
        raise
...
@tool
def kegg_pathway_search(query_list: List[str], model_name: str = "gemini-3") -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        model_name: Model name for LLM analysis, defaults to "gemini-3"
    ...
    try:
        ...
        pathway_df = pd.read_csv(os.path.join(RESOURCE_DIR, "Kegg_pathways.csv"))
...
def disease_gene_search(
    ...
    Args:
        ...
        file_path: Path to the disease mapping TSV file (defaults to RESOURCE_DIR/diseases/human_disease_integrated_full.tsv)
    
    Returns:
        Dictionary containing gene lists associated with each disease and normalized confidence scores
    """
    if not file_path:
        file_path = os.path.join(RESOURCE_DIR, "diseases", "human_disease_integrated_full.tsv")
...
def go_terms_search(
    ...
    json_directory = os.path.join(RESOURCE_DIR, "GO")
...
def tcga_survival_analysis(
    ...
    _tcga_dir = os.path.join(RESOURCE_DIR, "TCGA")
    data_files = {
        "CNA": os.path.join(_tcga_dir, "Table S1 - univariate Cox models_S1A - CNA.csv"),
        "Methylation": os.path.join(_tcga_dir, "Table S1 - univariate Cox models_S1B - DNA methylation.csv"),
        "Gene Expression": os.path.join(_tcga_dir, "Table S1 - univariate Cox models_S1C - Gene expression.csv"),
        "miRNA": os.path.join(_tcga_dir, "Table S1 - univariate Cox models_S1D - miRNA expression.csv"),
        "Mutations": os.path.join(_tcga_dir, "Table S1 - univariate Cox models_S1E - Mutations.csv"),
    }
...
def drug_gene_network_search(
    ...
    Args:
        ...
        graph_path: Path to the pickled graph file (defaults to RESOURCE_DIR/RxGrid/G_full.p)
        model_name: Model name for LLM analysis, defaults to "gemini-3"
    ...
    if not graph_path:
        graph_path = os.path.join(RESOURCE_DIR, "RxGrid", "G_full.p")
...
@tool
def hpo_phenotype_search(phenotype_terms: List[str], model_name: str = "gemini-3") -> Dict[str, Any]:
    """
...
@tool
def omim_disease_search(disease_terms: List[str], model_name: str = "gemini-3") -> Dict[str, Any]:
    """
...
@tool
def orphanet_rare_disease_search(disease_terms: List[str], model_name: str = "gemini-3") -> Dict[str, Any]:
    """
...
def gsea_hallmark_search(
    ...
    Args:
        ...
        json_directory: Path to directory containing MSigDB JSON files (defaults to RESOURCE_DIR/GSEA)
        model_name: Model name for LLM analysis, defaults to "gemini-3"
    ...
    if not json_directory:
        json_directory = os.path.join(RESOURCE_DIR, "GSEA")
...
def wikipathways_search(
    ...
    Args:
        ...
        json_directory: Path to directory containing WikiPathways JSON files (defaults to RESOURCE_DIR/WikiPathways)
        model_name: Model name for LLM analysis, defaults to "gemini-3"
    ...
    if not json_directory:
        json_directory = os.path.join(RESOURCE_DIR, "WikiPathways")
...
def reactome_pathway_search(
    ...
    Args:
        ...
        json_directory: Path to directory containing Reactome JSON files (defaults to RESOURCE_DIR/Reactome)
        model_name: Model name for LLM analysis, defaults to "gemini-3"
    ...
    if not json_directory:
        json_directory = os.path.join(RESOURCE_DIR, "Reactome")
...
def cancer_biomarkers_search(
    ...
    Args:
        ...
        json_path: Path to Cancer Biomarkers JSON file (defaults to RESOURCE_DIR/Open_target/cancerbiomarkers.json)
        model_name: Model name for LLM analysis, defaults to "gemini-3"
    ...
    if not json_path:
        json_path = os.path.join(RESOURCE_DIR, "Open_target", "cancerbiomarkers.json")
...
def clingen_search(
    ...
    Args:
        ...
        json_path: Path to ClinGen JSON file (defaults to RESOURCE_DIR/Open_target/clingen.json)
        model_name: Model name for LLM analysis, defaults to "gemini-3"
    ...
    if not json_path:
        json_path = os.path.join(RESOURCE_DIR, "Open_target", "clingen.json")
...
def gene2phenotype_search(
    ...
    Args:
        ...
        json_path: Path to Gene2Phenotype JSON file (defaults to RESOURCE_DIR/Open_target/gene2phenotype.json)
        model_name: Model name for LLM analysis, defaults to "gemini-3"
    ...
    if not json_path:
        json_path = os.path.join(RESOURCE_DIR, "Open_target", "gene2phenotype.json")
...
def gene_burden_search(
    ...
    Args:
        ...
        json_path: Path to Gene Burden JSON file (defaults to RESOURCE_DIR/Open_target/gene_burden.json)
        model_name: Model name for LLM analysis, defaults to "gemini-3"
    ...
    if not json_path:
        json_path = os.path.join(RESOURCE_DIR, "Open_target", "gene_burden.json")
...
def intogen_search(
    ...
    Args:
        ...
        json_path: Path to IntOGen JSON file (defaults to RESOURCE_DIR/Open_target/intogen.json)
        model_name: Model name for LLM analysis, defaults to "gemini-3"
    ...
    if not json_path:
        json_path = os.path.join(RESOURCE_DIR, "Open_target", "intogen.json")
...
def gocc_cellular_component_search(
    ...
    Args:
        ...
        json_directory: Path to directory containing GOCC JSON files (defaults to RESOURCE_DIR/GOCC)
        model_name: Model name for LLM analysis, defaults to "gemini-3"
    ...
    if not json_directory:
        json_directory = os.path.join(RESOURCE_DIR, "GOCC")
...
def clinvar_search(
    ...
    Args:
        ...
        json_path: Path to ClinVar JSON file (defaults to RESOURCE_DIR/Open_target/eva.json)
    ...
    if not json_path:
        json_path = os.path.join(RESOURCE_DIR, "Open_target", "eva.json")
...
def uniprot_variants_search(
    ...
    Args:
        query_list: List of disease names or related conditions to search for associated genes
        json_path: Path to UniProt variants JSON file (defaults to RESOURCE_DIR/Open_target/uniprot_variants.json)
        model_name: Model name for LLM analysis, defaults to "gemini-3"
    ...
    if not json_path:
        json_path = os.path.join(RESOURCE_DIR, "Open_target", "uniprot_variants.json")
